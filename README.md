...
* [上海电信 IPTV 教程](#上海电信-iptv-教程)
  ...
  * [2. 光猫设置](#2-光猫设置)
    ...
    * [2.2 IGMP 设置](#22-igmp-设置)
      * [2.1.1 电信公版界面](#211-电信公版界面-1)
      * [2.1.2 华为界面](#212-华为界面-1)
  * [3. 转发网关设置](#3-转发网关设置)
...
因此对于没有光猫超密但是已下发 ITV 口配置的用户，可直接跳过这一章，直接使用 ITV口 直接进入 [3. 转发网关设置](#3-转发网关设置) 部分。
...
设置成功后会有 [x]_Other_B_VID_85 的网络, 对此网络进行端口绑定，此操作在 网络-网络设置-VLAN设置中进行：
...
对于 UDPXY 和 msd_lite 除了 下文 3.3 节中的转发软件设置不同外其余部分完全相同，对于这一不同部分可参考此知乎文章中 msd_lite 进行设置：
[体验msd_lite替换udpxy](https://zhuanlan.zhihu.com/p/1906718559213688181)

### 3.1 VLAN85 DHCP 获得 IP
转发网关首先需要在 VLAN85 上获得 IP, 在 [2.1 VLAN 85 桥接与 VLAN 51 组播绑定](#21-vlan-85-桥接与-vlan-51-组播绑定) 的光猫设置中，我们已经设置了 VLAN85 的桥接，我们直接 DHCP 即可获得 IP
