...
> [!NOTE]
>
> 在设置光猫的同时最好去掉光猫本身的 DHCP, 否则后续 IPTV 网关 DHCP获得 IP 时，猫自身的 DHCP 优先级会高于端口的 VLAN 85 DHCP
>
> 由于使用 ITV 口的用户基本都无超密，故无法关闭猫自身的 DHCP。为此请在下文 [3.1 VLAN85 DHCP 获得 IP](#31-vlan85-dhcp-获得-ip) 进行操作时，记得查看 NOTE 部分。
...
> [!NOTE]
>
> 如果在前述光猫设置阶段未能关闭光猫 DHCP, 此处直接 DHCP 会获得光猫分发的 IP
>
> 此时需要在拨号时手动选择在 VLAN 85 上获得 IP，对于 openwrt 需要在物理设置中设置：
> ![force_vlan85_dhcp.PNG](pic/force_vlan85_dhcp.PNG)
> 此处以 eth1 为例，实际使用时基于你的网口使用 eth[x].85 即可


获得 30 开头的 IP 即成功接入 IPTV 网关
